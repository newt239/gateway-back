...
router.post('/:activity_type', verifyToken, function (req: express.Request, res: express.Response) {
    ...
    if (["enter", "exit", "pass"].indexOf(activity_type, -1)) {
        ...
        if (activity_type === "enter") {
            sql += `INSERT INTO gateway.session (session_id, exhibit_id, guest_id, guest_type, enter, available) VALUES ('s${activity_id}', '${exhibit_id}', '${guest_id}', '${guest_type}', '${timestamp}', 1);`
        } else if (activity_type === "exit") {
            sql += `UPDATE gateway.session SET exit='${timestamp}' WHERE guest_id='${guest_id}' AND exhibit_id='${exhibit_id}' AND exit=NULL;`;
        } else if (activity_type === "pass") {
            sql += `INSERT INTO gateway.session (session_id, exhibit_id, guest_id, guest_type, enter, exit, available) VALUES ('s${activity_id}', '${exhibit_id}', '${guest_id}', '${guest_type}', '${timestamp}', '${timestamp}', 1);`
        }
